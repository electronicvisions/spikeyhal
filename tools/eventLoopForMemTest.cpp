// copy of tests/eventLoop.cpp
// TODO: reduce for mem tests
...
{
	std::string filename = "gtest_log_elb.txt";
	log4cxx::LayoutPtr layout = new log4cxx::SimpleLayout();
	log4cxx::AppenderPtr file = new log4cxx::FileAppender(layout, filename, false);
	log4cxx::BasicConfigurator::configure(file);
	static log4cxx::LoggerPtr logger = log4cxx::Logger::getLogger("Tst.Mem");
	...
	for (uint l = 0; l < loops; l++) {
		...
		if (!checkEvents(error, st_tx, st_rx, sc))
